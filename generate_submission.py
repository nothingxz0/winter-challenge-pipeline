...
def generate_cpp(data):
    ...
    encoded, orig_len, table = encode_ascii85(total_blob)
    print(f"Blob: {len(total_blob)} bytes -> {len(encoded)} chars (ascii85)")
    ...
    P.append(f'static const int BS={bias_start};\n')
    ...
    # ---- Decode function ----
    P.append('static void decW(){unsigned char r[80000];const char*p=ED;int n=0;\n')
    ...
    P.append(f'memcpy(AB,r+BS,{total_bias * 4});\n')
